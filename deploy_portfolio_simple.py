#!/usr/bin/env python3
"""
Simple deployment script for portfolio to Azure using scp
"""
...
def deploy_portfolio():
    """Deploy portfolio to Azure using scp"""
    ...
    if not run_command(scp_index_cmd, "Copying index.html"):
        return False
    ...
    # Copy mark directories using scp
    ...
    for mark_dir in mark_dirs:
        if os.path.exists(mark_dir):
            ...
            # Copy files using scp (recursive)
            scp_cmd = f"scp -i {key_path} -o StrictHostKeyChecking=no -r {mark_dir}/* {username}@{server_ip}:/var/www/html/{mark_dir}/"
            if run_command(scp_cmd, f"Copying {mark_dir} files"):
                # Set permissions
...
def main():
    print("🚀 Simple Portfolio Azure Deployment Script")
    print("=" * 40)

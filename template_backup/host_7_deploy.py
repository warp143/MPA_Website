#!/usr/bin/env python3
"""
Simple rsync deployment for mark7 to Azure
"""
...
def deploy_mark7():
    ...
    print(f"📁 Source: mark7/")
    print(f"📁 Target: /var/www/html/mark7/")
    print("=" * 50)
    
    # Verify local mark7 directory exists
    if not os.path.exists("mark7"):
        print("❌ Local mark7 directory not found")
        return False
    ...
    remove_cmd = f"{ssh_cmd} 'sudo rm -rf /var/www/html/mark7'"
    run_command(remove_cmd, "Removing old directory")
    
    mkdir_cmd = f"{ssh_cmd} 'sudo mkdir -p /var/www/html/mark7 && sudo chown azureuser:azureuser /var/www/html/mark7'"
    run_command(mkdir_cmd, "Creating target directory")
    
    # Copy files using rsync
    rsync_cmd = f"rsync -avz --delete -e 'ssh -i {key_path} -o StrictHostKeyChecking=no' mark7/ {username}@{server_ip}:/var/www/html/mark7/"
    if not run_command(rsync_cmd, "Copying files to server"):
        return False
    
    # Set proper permissions
    chmod_cmd = f"{ssh_cmd} 'sudo chown -R www-data:www-data /var/www/html/mark7/ && sudo chmod -R 644 /var/www/html/mark7/* && sudo chmod 755 /var/www/html/mark7/ && sudo chmod 755 /var/www/html/mark7/assets/'"
...
def main():
    print("🚀 Mark7 Azure Deployment Script (rsync)")
    print("=" * 40)
    
    # Check if mark7 directory exists
    if not os.path.exists("mark7"):
        print("❌ Error: mark7/ directory not found in current location")
        print("   Please run this script from the Website directory")
        sys.exit(1)
    
    # Show deployment info
    print(f"📁 Source directory: {os.path.abspath('mark7')}")

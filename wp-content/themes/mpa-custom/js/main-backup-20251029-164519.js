...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.language-option').forEach(option => {
        option.addEventListener('click', function() {
            ...
            // Use Polylang URLs if available
            if (window.polylangUrls && window.polylangUrls[lang]) {
                window.location.href = window.polylangUrls[lang];
                return;
            }
            
            // Fallback to old client-side method
            localStorage.setItem('selectedLanguage', lang);
            currentLang = lang;
            applyTranslations();

...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Translation data - now loaded from API via MPA_TRANS plugin

    // Translation data - now loaded from API via MPA_TRANS plugin
    // OLD: Removed 351 lines of hardcoded translations (was lines 214-565)
    // NEW: Translations fetched from /wp-json/mpa/v1/translations/{lang}
    
    // Load translations from API with caching
    async function loadTranslations(lang) {
        if (!window.MPA_TRANS) {
            console.error('MPA_TRANS not loaded!');
            return {};
        }

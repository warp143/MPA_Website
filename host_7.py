...
Hosts the mark7 directory on port 1734
"""
...
def main():
    ...
    if not mark7_dir.exists():
        print(f"❌ Error: mark7 directory not found at {mark7_dir}")
        print("Please make sure the mark7 directory exists in the same location as this script.")
        sys.exit(1)
    ...
    for retry in range(max_retries):
        try:
            with socketserver.TCPServer(("", PORT), Handler) as httpd:
                ...
                print(f"📁 Serving directory: {mark7_dir}")
                print(f"🌐 Server URL: http://localhost:{PORT}")
                print(f"📄 Main page: http://localhost:{PORT}/index.html")
                print(f"🎨 WordPress theme: http://localhost:{PORT}/mpa-wordpress-theme/")
